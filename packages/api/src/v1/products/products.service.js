...
const getProducts = function(done) {
  ...
  ProductModel.find(query)
    ...
    .exec((err, colln) => {
      if (err) {
        console.error(
          'Error in finding products, ERROR::',
          err,
          ' queries for ',
          query
        );
...
const findProductByCode = function(productCode, done) {
  ...
  ProductModel.findOne(query)
    .select('name code vendor price')
    .exec((err, productData) => {
      if (err) {
        console.error('No product could be found. ERROR::', err);
        done(err);
        return;
      }
...
const submitNewReview = function(productCode, reviewObj, done) {
  ...
  ProductModel.findOneAndUpdate(query, modification, options, function(
    err,
    updatedDoc
  ) {
    if (err) {
